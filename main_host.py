...
bot = telebot.TeleBot("7698859036:AAGruVW2S8m0Dkl89aS8qBRbaHdkVrBvMDM")

# Registered users dictionary
registered_users = {}

# List of wallet names to scan
wallets = {
...
# Function to retrieve wallet signatures
...
# Function to retrieve the sender address for a signature
...
def get_bearer_token():
    try:
        with open("config.txt", "r") as file:
            return file.read().strip()  # Read and strip any extra whitespace or newline
...
# Function to scan wallets with stop functionality
    ...
    while scanning:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for source in sources:
                if not scanning:
                    return  # Stop processing immediately if scanning is set to False
...
# Admin check decorator (applied to most commands, except /replace_admin_list)
...
def handle_config(message):
    ...
    # Extract the config data from the message
    try:
        config_data = message.text.split(" ", 1)[1]  # Everything after the first space
    except IndexError:
        bot.reply_to(message, "No configuration data provided. Please send /config {config_data}.")
        return

    # Save the config data to the config.txt file
    try:
